...
legend_native_class <- get_legend(rank_abu_fig + theme(legend.position="bottom"))

rank_abu_figs <- plot_grid(rank_abu_fig, rank_abu_NN_fig,  labels = "auto")

plot_grid( rank_abu_figs, legend_native_class, ncol = 1, rel_heights = c(1, .05))
```
...
We can use PERMANOVA analyses to test whether ant community composition varied between seasons and years. PERMANOVA tests for differences in the locations of multivariate groups. We used the adonis function of vegan. As we had repeated community measures within site, we constrained permuatations to witin sites (using the "strata" argument). We included season and year as fixed effects in the models. These analyses confirmed that season had a significant effect on community composition and explained approximately 5% of the variation (R2 = 0.055)
...
anova(lmer_inv_0, lmer_inv_fit, test="Chisq")
...
dist_edge_fig_non_natives <-ggplot(data_MH_all_invasives,aes(dist_km, abundance))+
  ...
  theme_classic()+
  #theme(legend.position = "bottom")+
  labs(x = "Distance to preseve boundary (km)",
       y = "Ant abundance")
...
Each point represents whether or not a non-native ant species was detected in the sample, shapes represent the method of collection (Hand = circles, Pitfall traps = triangles, and Winkler = squares). The fitted line is the Binomial model estimates for the probability of the presence of a non-native ant species within the trap, with site as a random effect. 
...
Here, each point represents the abundance of *N. fulva* (tan) or *S. invicta* (blue) by collection method (Hand = circles, Pitfall traps = triangles, Winkler = squares) at a given sampling site that is some distance (km) to the nearest preserve boundary. The line is the best fit regression (r2 = .026) with site as a random effect.

## Next steps
- Decide whether to classify *P. falvens* complex as native or non-native.
- Decide whether it makes more sense to present alpha diversity indices based on *occurrence* or *abundance*. (*I think abundance makes more sense, since the Shannon and Simpson indicies are abundance based*).
